...
def build_seed_stock_data(info: dict) -> dict:
    ...
    return {
        ...
        "company": {"pe_percentile":30,"pb_percentile":35,"ps_ratio":3,"peg_ratio":1,"ev_ebitda":10,"roe_5y":18 if hd else 22,"roic":12 if hd else 16,"gross_margin":55 if hd else 65,"net_margin":20 if hd else 30,"op_cashflow":12 if hd else 15,"market_share":55,"cost_advantage":50,"brand_power":moat,"channel":55,"patents":40,"barrier":moat,"overseas_revenue_pct":g//2,"overseas_orders":g//3,"global_clients":g//2,"overseas_capacity":g//4},
        ...
        "timing": {"pe_percentile":pp,"current_pe":pv,"industry_avg_pe":ip,"industry_growth":ig,"pb_percentile":max(0.05,0.40-d*0.03),"dividend_yield":d/100.0},
